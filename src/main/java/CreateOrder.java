...
public class CreateOrder {
    ...
    //System.getProperty("user.dir") + File.separator + "Order.xls"
    public static void createOrder(Order order) {
        try {
            ...
            //外循环的次数和购物车的实际长度
            for(int k = 1;k <= order.getProduct().length;k++){
                ...
                for(int j = 0;j < 6;j++){
                    ...
                    if(j == 0){
                        ...
                        cell.setCellValue(order.getProduct()[k-1].getProId());
                    }else if(j == 2){
                        cell.setCellValue(order.getAmount().get(order.getProduct()[k-1])+"");
                    }else if(j == 3){
                        cell.setCellValue(order.getProduct()[k-1].getPrice() * order.getAmount().get(order.getProduct()[k-1]) +"");
                    }else if (j == 4){
                        cell.setCellValue(order.getProduct()[k-1].getPrice() * order.getAmount().get(order.getProduct()[k-1]) +"");
                    }else{
                        SimpleDateFormat formatter = new SimpleDateFormat("yyyy年-MM月dd日-HH时mm分ss秒");
                        cell.setCellValue(formatter.format(order.getOrderDate()));
                    }
                }
            ...
            System.out.println("已运行 CreateOrder() : " + e);
        }
    }
}